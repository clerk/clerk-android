  ...
  override fun tokens(): ButtonStyleTokens {
    ...
    val foreground =
      when (config.emphasis) {
        ...
        ClerkButtonConfig.Emphasis.Low -> clerkColors.danger!!
    ...
    val backgroundColor =
      when (config.emphasis) {
        ...
        ClerkButtonConfig.Emphasis.Low ->
          if (isPressed) computedColors.backgroundDanger else clerkColors.background!!
        ClerkButtonConfig.Emphasis.High ->
          if (isPressed) computedColors.backgroundDanger else clerkColors.danger!!
      }
    val borderWidth =
      when (config.emphasis) {
