 ...
 *
 * This middleware automatically adds the `_is_native` query parameter to all outgoing
 * HTTP requests to identify them as coming from a native mobile client. This allows
 * the Clerk API to provide mobile-specific responses and behavior.
 *
 * The interceptor modifies the request URL by adding the query parameter before
 * forwarding the request to the next interceptor in the chain.
 */
internal class UrlAppendingMiddleware : Interceptor {
  /**
   * Intercepts the request and appends the native client query parameter.
   *
   * This method modifies the original request URL by adding the `_is_native=true`
   * query parameter, then proceeds with the modified request.
   *
