...
internal interface SessionApi {
   ...
   *
   * This method removes the specified session from the client's active sessions list.
   * The session will no longer be available for authentication purposes.
   *
   * @param id The unique identifier of the session to remove
   * @return A [ClerkResult] containing the removed [Session] on success, or a [ClerkErrorResponse] on failure
   */
   ...
   *
   * This method removes all sessions associated with the current client, effectively
   * signing out the user from all devices and sessions.
   *
   * @return A [ClerkResult] containing the updated [Client] on success, or a [ClerkErrorResponse] on failure
   */
   ...
   *
   * This method fetches authentication tokens associated with the specified session,
   * which can be used for API authentication and authorization.
   *
   * @param sessionId The unique identifier of the session to get tokens for
   * @return A [ClerkResult] containing the [TokenResource] on success, or a [ClerkErrorResponse] on failure
   */
   ...
   *
   * This method fetches authentication tokens for a specific user using a template type,
   * which allows for customized token generation based on the template configuration.
   *
   ...
   *
   * This method revokes the specified session, making it invalid for future authentication.
   * The revoked session will no longer be usable for API calls or authentication.
   *
   ...
   * @return A [ClerkResult] containing the revoked [Session] on success, or a [ClerkErrorResponse] on failure
   */
