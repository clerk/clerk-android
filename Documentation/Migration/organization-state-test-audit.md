...
val suggestedSlug: String = defaults.form.slug
val activeId = clerk.organization?.id
val activeMembership = clerk.user?.organizationMemberships?.firstOrNull {
  it.organization.id == activeId
}
val canReadBilling = activeMembership?.permissions?.contains("org:sys_billing:read") == true
```
