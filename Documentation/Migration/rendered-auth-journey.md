...
The script requires the exact named passing XML case. It sets `additionalTestOutputDir` so Gradle collects screenshots before removing the test application, following [Android's test-output mechanism](https://developer.android.com/topic/performance/benchmarking/benchmarking-in-ci). The API 36 x86_64 CI job now invokes this script and archives its XML and screenshots; the [September 10 platform job](https://github.com/clerk/clerk-android/actions/runs/34503501613/job/102959826579) passed the journey. Its [XML](evidence/android-ci-20260910/TEST-api-36-auth-ui.xml), screenshots and hashes are retained in the [CI proof record](evidence/android-ci-20260910/proof.json).
