...
The native adapter reports a boolean to the embedded core. Apple permits requests when the path is satisfied. Android uses Internet/validated capabilities when available and ignores loss of a replaced default network. API 24/25 availability permits an HTTP attempt while capabilities are unknown because those versions can omit the follow-up capabilities event. API 26 and later wait for that event. Monitoring registration failure also leaves requests eligible. See the [API 24/25 regression and adapter proof](lifecycle-test-audit.md). These are OS observations, not proof of Clerk reachability; HTTP can still fail.
