...
import android.hardware.biometrics.BiometricManager as PlatformBiometricManager
...
internal data class BiometricCredentialLocalKey(
  ...
  val policy: BiometricCredentialPolicy = BiometricCredentialPolicy.BiometryOrDevicePasscode,
)
 ...
 */
internal class BiometricCredentialKeyManagerException
...
internal class AndroidBiometricKeyManager(private val context: Context, private val activityProvider: (() -> Activity?)?) : BiometricCredentialKeyManager {
  ...
  override fun isSupported(policy: BiometricCredentialPolicy): Boolean {
    ...
    val authenticators =
      if (
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.R &&
          policy == BiometricCredentialPolicy.BiometryOrDevicePasscode
      ) {
    ...
    return activityProvider != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P &&
      context != null &&
  ...
  private suspend fun authenticateWithBiometricPrompt(
    ...
    val activity =
      activityProvider?.invoke()?.takeUnless { it.isFinishing || it.isDestroyed }
        ?: throw BiometricCredentialKeyManagerException(
          BiometricCredentialKeyManagerException.Code.SIGNING_FAILED,
      ...
      suspendCancellableCoroutine { continuation ->
        ...
        if (allowsDeviceCredential) {
          builder.setAllowedAuthenticators(
            PlatformBiometricManager.Authenticators.BIOMETRIC_STRONG or
              PlatformBiometricManager.Authenticators.DEVICE_CREDENTIAL
          )
        } else {
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            builder.setAllowedAuthenticators(PlatformBiometricManager.Authenticators.BIOMETRIC_STRONG)
          }
  ...
  private fun keyGenParameterSpec(
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
      val authenticators =
        if (policy == BiometricCredentialPolicy.BiometryOrDevicePasscode) {
  ...
  private inner class DerReader(private val bytes: ByteArray) {
  ...
  private fun applicationContext(): Context? = context.applicationContext
}
