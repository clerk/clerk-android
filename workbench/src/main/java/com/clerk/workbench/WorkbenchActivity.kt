...
abstract class WorkbenchActivity : ComponentActivity() {
  private val app
    get() = application as WorkbenchApplication
          ...
          when {
            ...
            connection!!.isFailure ->
              ...
              ClerkProvider(connection!!.getOrThrow()) {
                ...
                if (owner.authCallback != null) AuthView(isDismissible = false) else content()
              }
          }
          val message = callbackError
          if (message != null)
            AlertDialog(
              onDismissRequest = {
