...
class WorkbenchApplication : Application() {
  ...
  fun connect() {
    ...
    scope.launch {
      try {
        val key =
          StorageHelper.loadValue(StorageKey.PUBLIC_KEY)?.takeIf { it.isNotBlank() }
            ?: throw IllegalStateException("Open Settings and enter a publishable key.")
        if (!StorageHelper.loadValue(StorageKey.PROXY_URL).isNullOrBlank()) {
          throw IllegalStateException(
            "Proxy URLs are unavailable in this native core prerelease. Clear the proxy URL in Settings."
          )
        }
        _connection.value =
          Result.success(
            Clerk.connect(
              this@WorkbenchApplication,
              ClerkConfiguration(
                key,
